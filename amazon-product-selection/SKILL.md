...
## 联动工具（已安装 dsh-shop-assistant 插件后适用）

- **可复算评分**：当需要"成本/售价/竞争度"固定公式评分、且已安装 dsh-shop-assistant 时，优先用 `shop_product_score` 计算（同样的输入 → 同样的输出，不靠模型臆测），再把结果并入本 skill 的多维筛选结论。
- **竞品页快照**：选品调研需要看竞品页面时，可用 `shop_page_snapshot` 抓公开商品页，作为卖家精灵/ABA 数据的补充（注意页面价格只是"线索"，不以它为成交价断言）。
- **切记**：shop_* 的结果是**输入之一**，最终结论仍须按本 skill 的维度框架（趋势/竞争/利润/供需）交叉验证，不能只凭单一工具下结论。
