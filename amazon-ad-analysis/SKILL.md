...
- **CSV/Excel 批量预览**：优先用 `shop_csv_preview` 读取用户上传的广告/产品/搜索词表（整表一次通过，不要求用户手粘），再按本 skill 的四份文件数据架构分析。
- **分平台列适配**：`shop_csv_preview` 默认按淘宝/拼多多列适配；Amazon 的列（如 `campaign`、`clicks`、`spend`、`orders`、`ACOS`、`CPC`、`CTR`、`CVR`）需先做表头映射，映射不齐时向用户确认，不要臆断列含义。
- **列映射建立后**：把映射关系记入该次分析报告头部，便于复用与复核。
- `shop_*` 工具不可用或无法正确适配 Amazon 列时，回退到本 skill 原有"读取工作区文件 + 逐行核对"流程。
...
4. **搜索词数据预处理**（搜索词报告专用）：
   ...
   - 过滤方法：`if row['开始日期'] != row['结束日期']: skip`
   - 校验：单日行的花费总和应接近广告表现文件的SP花费总和
5. **数据校验**：
...
| 优先级 | 方式 | 工具 | 适用场景 |
|--------|------|------|----------|
> **DSH 说明**：DSH 无浏览器自动化与 `webfetch` 工具。前台 Listing 抓取降级方案调整为：
>
> | 优先级 | 方式 | 工具 | 适用场景 |
> |--------|------|------|----------|
> | 1 | Jina Reader | `curl 'https://r.jina.ai/http://amazon.com/dp/{ASIN}'` | 默认 |
> | 2 | curl 直抓 | `curl` Amazon URL（需处理反爬，成功率低） | Jina 不可用时 |
> | 3 | 手动输入 | 用户提供 Listing 文本 | 自动方式失败时 |
> | 4 | 跳过 | 标注"前台数据未获取" | 无法获取时 |
...
- 过滤方法：`if row['开始日期'] != row['结束日期']: skip`
- 校验：单日行的花费总和应接近广告表现文件的总花费（偏差<10%为正常）
...
| 文件 | 用途 |
|------|------|
| `scripts/analysis.py` | 可执行脚本：词根分析、否定词生成、关键词覆盖分析、数据清洗、数据验证 |
| `references/metrics-glossary.md` | 指标定义与健康标准（ACOS 统一阈值） |
