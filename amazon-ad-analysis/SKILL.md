...
| 条件 | 标记 | 经营含义与动作 |
|------|------|----------------|
| Top1 > `top1_spend_share_risk` **或** HHI > `hhi_risk` | 🔴 **单点依赖** | 主力活动波动 → 整体广告同步波动。**放量前先分散结构**（拆活动/拓已验证词/引入第二梯队），不得只加预算 |
| Top1 ≤ 阈值 且 Top3 > `top3_watch` | 🟡 头部集中 | 抗风险尚可，但第二梯队缺席，注意单点化趋势 |
| 其余 | 🟢 分散 | 结构健康，集中度不构成约束 |

**输出**：

| ASIN | 活动数 | 总花费 | Top1活动 | Top1占比 | Top3占比 | HHI | 标记 |
|------|--------|--------|----------|----------|---------|-----|------|
| {ASIN} | {n} | ${x} | {活动名} | {x}% | {x}% | {x.xxx} | 🔴/🟡/🟢 |
...
5.1=词根表现汇总（依赖 4C）；5.2=否定词清单（依赖 4C）；5.3=关键词覆盖分析（依赖 4A+4C）；5.4=付费-自然结构诊断（依赖 4C + 2B，B级需卖家精灵 overlay）。
...
| **B 级** | A 级 + 卖家精灵插件「反查关键词」面板（字段见 5.4-B） | A 级全部 + 5.4-B 自然位缺口 + 5.4-C（完整判据） |
...
**输入**：
① 搜索词报告按词聚合（点击 / 花费 / 订单）
② 卖家精灵插件「**反查关键词**」面板（**字段名已于 2026-09-17 实测固化**，见下表）

**实测取数路径（2026-09-17 验证通过）**：Amazon 商品页 → 卖家精灵插件悬浮面板 → 「反查关键词」。
...
**Excel附件检查**：
- [ ] 已创建Excel文件（产品概览、广告类型对比、搜索词Top10、词根表现汇总、否定词执行清单、关键词覆盖分析、广告集中度、付费-自然结构）
- [ ] Excel文件已保存到正确路径：`{VAULT_PATH}/工作/亚马逊工作管理/附件/4.ASIN深入分析/{ASIN} 分析数据.xlsx`
- [ ] Markdown报告中的附件链接可正确打开
- [ ] **产出规格漂移检查（2026-09-17 新增）**：逐项核对实际生成的 sheet 名与结构与本节规格是否一致，**不一致必须在报告中声明差异**。实测警示：vault 内 19 个历史 workbook 存在"规范版 / 中文变体版 / 残缺版（仅周趋势+搜索词）"三套并存 → 禁止默认"已按规格生成"。
...
**Sheet 8: Ad Concentration**（2026-09-17 新增，Phase 2D 输出）
- 来源：广告表现日度文件（降级：搜索词报告）
- 包含：广告活动、28天花费、花费占比、累计占比、集中度标记（🔴/🟡/🟢）
- 末行汇总：Top1占比 / Top3占比 / HHI / 标记 + 口径声明

**Sheet 9: Paid-Natural**（2026-09-17 新增，Phase 5.4 输出）
- 区块 A：四象限（广告花费 W3→W4、自然订单 W3→W4、象限、动作）
- 区块 B：自然位缺口清单（B 级才有；无 overlay 时留空并标注"A/B/C 级别"）
- 区块 C：四态分层（词根、词态、判据级别、依据、建议）
